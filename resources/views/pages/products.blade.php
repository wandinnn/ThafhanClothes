...
new class extends Component
{
    ...
    public ?string $searchSuggestion = null;
    ...
    /**
     * Query dasar produk lengkap dengan seluruh filter yang sedang aktif,
     * dipakai baik untuk mode telusur maupun mode pencarian.
     */
    ...
    /**
     * Urutkan hasil pencarian sesuai pilihan pengguna. Saat pengguna memakai
     * urutan bawaan, relevansi hasil pencarian yang dipertahankan.
     *
     * @param  array<int, Product>  $products
     * @return array<int, Product>
     */
    private function sortSearchResults(array $products): array
    {
        $comparator = match ($this->sortBy) {
            'price-low-high' => fn (Product $a, Product $b): int => $a->price <=> $b->price,
            'price-high-low' => fn (Product $a, Product $b): int => $b->price <=> $a->price,
            'name-asc' => fn (Product $a, Product $b): int => strcasecmp($a->name, $b->name),
            'name-desc' => fn (Product $a, Product $b): int => strcasecmp($b->name, $a->name),
            'popular' => fn (Product $a, Product $b): int => ($b->is_best_seller <=> $a->is_best_seller),
            default => null,
        };
     ...
     * @param  array<int, Product>  $products
     */
    private function paginateResults(array $products, int $perPage, int $page): LengthAwarePaginator
    {
    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
            <div class="flex items-center gap-2">
                <button wire:click="$toggle('showFilters')"
                    ...
                    @if($this->hasActiveFilters)
                        <span class="bg-beige text-deep text-xs font-bold rounded-full w-4.5 h-4.5 flex items-center justify-center leading-none">!</span>
                    @endif
                </button>
                @if($this->hasActiveFilters)
            ...
            <div class="flex items-center gap-2">
                ...
                <select id="sortBy" wire:model.live="sortBy"
                        class="rounded-xl border border-gray-200 bg-panel px-4 py-2 text-sm text-gray-700 shadow-sm focus:border-beige focus:outline-none focus:ring-2 focus:ring-beige/20">
        ...
        @if(trim($search) !== '')
            @if($searchSuggestion)
                <div class="mb-4 text-sm text-gray-700">
                    <span class="text-ink">Mungkin yang Anda maksud:</span>
                    <button type="button" wire:click="applySuggestion(@js($searchSuggestion))" class="font-semibold text-deep ml-2">{{ $searchSuggestion }}</button>
                </div>
            @else
                <div class="mb-4 text-sm text-ink">Menampilkan hasil yang paling mendekati pencarian Anda</div>
            @endif
        @endif

        {{-- Filter Panel --}}
        @if($showFilters)
            <div class="bg-panel rounded-2xl border border-gray-100 shadow-sm p-5 mb-6 animate-fade-up">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">

                    {{-- Price Range --}}
                    <div>
                        <p class="text-xs font-semibold text-ink uppercase tracking-wider mb-3">Rentang Harga</p>
                        <div class="flex items-center gap-2">
                            <div class="flex-1">
                                <label class="text-xs text-ink mb-1 block">Min (Rp)</label>
                                <input wire:model.blur="priceMin"
                                       type="number" min="0" step="10000"
                                       placeholder="0"
                                       class="w-full px-3 py-2 text-sm rounded-lg border border-gray-200 focus:border-beige focus:outline-none focus:ring-2 focus:ring-beige/20">
                            </div>
                            <span class="text-ink mt-5">—</span>
                            <div class="flex-1">
                                <label class="text-xs text-ink mb-1 block">Max (Rp)</label>
                        ...
                        @if($priceMin > 0 || $priceMax > 0)
                            <p class="text-xs text-deep mt-2 font-medium">
                                @if($priceMin > 0 && $priceMax > 0)
                        ...
                        <div class="space-y-2.5">
                            <label class="flex items-center gap-2.5 cursor-pointer group">
                                <input wire:model.live="filterBestSeller" type="checkbox"
                                       class="w-4 h-4 rounded border-gray-300 text-deep focus:ring-beige/20 cursor-pointer">
                                <span class="text-sm text-gray-700 group-hover:text-deep transition-colors flex items-center gap-1.5">
                            ...
                            <label class="flex items-center gap-2.5 cursor-pointer group">
                                <input wire:model.live="filterFlashSale" type="checkbox"
                                       class="w-4 h-4 rounded border-gray-300 text-deep focus:ring-beige/20 cursor-pointer">
                                <span class="text-sm text-gray-700 group-hover:text-deep transition-colors flex items-center gap-1.5">
                                    <span class="inline-block w-2.5 h-2.5 rounded-full bg-beige"></span>
                                    Flash Sale
                                </span>
                            </label>
                        </div>
                    ...
                    <div>
                        <p class="text-xs font-semibold text-ink uppercase tracking-wider mb-3">Filter Aktif</p>
                        @if($this->hasActiveFilters)
                            <div class="flex flex-wrap gap-1.5">
                                @if($priceMin > 0)
                                    <span class="inline-flex items-center gap-1 bg-coral/10 text-coral text-xs font-medium px-2.5 py-1 rounded-full border border-coral/25">
                                        Min Rp{{ number_format($priceMin, 0, ',', '.') }}
                        ...
                        @else
                            <p class="text-sm text-ink">Belum ada filter aktif.</p>
                        @endif
                    </div>
                </div>
            </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-5">
            @forelse($products as $product)
                <div class="product-card bg-panel rounded-xl overflow-hidden shadow-sm">
                    ...
                    <div class="p-3 sm:p-4">
                        <a wire:navigate href="{{ route('product.detail', $product->slug) }}">
                            <p class="text-[10px] sm:text-xs text-ink uppercase tracking-wide">{{ $product->category->name }}</p>
                            <h3 class="mt-0.5 text-sm font-semibold text-gray-900 transition-colors line-clamp-1">{{ $product->name }}</h3>
                            <div class="mt-1 flex items-center gap-1.5 flex-wrap">
                                <span class="text-deep font-bold text-sm">{{ $product->formatted_price }}</span>
                            ...
                            @if($product->reviews_count > 0)
                                <div class="flex items-center gap-1 mt-1">
                                    ...
                                    <span class="text-[10px] text-ink font-medium">{{ $product->average_rating }} ({{ $product->reviews_count }})</span>
                                </div>
                            @endif
                        </a>
            ...
            @empty
                <div class="col-span-full py-16 text-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="size-14 text-ink mx-auto mb-3" fill="none" viewBox="0 0 24 24" stroke-width="1" stroke="currentColor">
                    ...
                    <p class="text-ink font-medium">Tidak ada produk ditemukan.</p>
                    @if($this->hasActiveFilters)
                        <button wire:click="resetFilters" class="mt-3 text-sm text-deep hover:underline">Reset semua filter</button>
                    @endif
                </div>
            @endforelse
        </div>
